...
if len(team_filter) > 1:
    ...
    filtered_df['Game Label'] = filtered_df['TeamNM'] + ' vs ' + filtered_df['Opponent']
    x_axis = 'Game Number'
    x_label = 'Game Number (Team vs Opponent)'
else:
    x_axis = 'Date'
    x_label = 'Date'
    ...
    for column in selected_columns:
        ...
        if graph_type == "Bar Chart":
            fig = px.bar(filtered_df, x=x_axis, y=column, color='TeamNM',
                         text='Game Label' if len(team_filter) > 1 else 'Opponent',
            ...
            fig = px.scatter(filtered_df, x=x_axis, y=column, color='TeamNM', title=f'{stat_name} by {x_label}',
                             trendline="ols", color_discrete_map=color_map)
